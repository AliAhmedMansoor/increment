...
late String habitName='New Habit'; ///will be changed in the future
...
class _HabitSettingsState extends State<HabitSettings> {
  ...
  void setUpPreferences()async{
    if(widget.habitNumber!=null) {
      ...
      routine = preferences.getStringList('routine$habitNumber') as List<String>;
      List<String> tempList=(preferences.getStringList('habitStart$habitNumber') as List<String>);

      habitStart=habitStart.replacing(hour: int.parse(tempList[0]),minute: int.parse(tempList[1]));
      tempList=preferences.getStringList('habitDuration$habitNumber') as List<String>;

      habitDuration=Duration(minutes: int.parse(tempList[0]),hours: int.parse(tempList[1]));
...
class EditPrompt extends StatelessWidget {
  ...
  void savePreferences()async{
    ...
    await preferences.setStringList('habitStart$habitNumber', [habitStart.hour.toString(),habitStart.minute.toString()]);
    if(habitDuration.inHours==0) {
      await preferences.setStringList('habitDuration$habitNumber', [habitDuration.inMinutes.toString(),'0']);
    }
    else{
      await preferences.setStringList('habitDuration$habitNumber', ['0',habitDuration.inHours.toString()]);
    }
  }
      ...
      child: Stack(
          ...
          children: [
            ...
            const Positioned(
                ...
                right: 0,
                height: 80,
                child: Divider(endIndent: 50,indent: 50,thickness: 0.5,height: 0,color: Colors.white,)
            ),
    ...
    return Material(
      ...
      child: SizedBox(
        ...
        child: const Row(
          ...
          children: [
            VerticalTimeListView(screenWidthRatio: 0.5,includeZero: true,lastNumber: 23,),
            Text(':',style: TextStyle( color: Colors.white,fontSize: 50,fontWeight: FontWeight.bold),),
            VerticalTimeListView(screenWidthRatio: 0.4,includeZero:true,lastNumber: 60,),
          ],
        ),
      ),
    );
...
class Routine extends StatelessWidget {
  ...
  static List<RoutineUnit> DaysofWeek=List.generate(7, (index) => RoutineUnit(day: days[index],));
  @override
  Widget build(BuildContext context) {
    return Material(
      color: const Color.fromARGB(255, 28, 33, 41),
      child: Column(
        children: [
          const Text('Routine',style:TextStyle(color: Colors.white,fontSize: 22),),
          ...
          Table(
            children: [
              ...
              TableRow(
                  children: [DaysofWeek[1],DaysofWeek[3],DaysofWeek[5]]
              ),
              TableRow(
                  children: [const SizedBox(),const SizedBox(),DaysofWeek[6]]
              ),
...
class _RoutineUnitState extends State<RoutineUnit> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Align(
          heightFactor: 2,
          child: Text(widget.day,textAlign: TextAlign.center,style: TextStyle(fontStyle:FontStyle.italic,fontSize: 12,color:currentColor),)),
    );
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: Column(
        ...
        children: [
          const Text('Habit Duration',style: TextStyle(fontSize: 28,fontWeight: FontWeight.bold,color: Colors.white),),
          const Divider(thickness: 0.5,height:0,indent: 70,endIndent: 70,color: Colors.white,),
          Align(
            heightFactor: 2,
            child: Row(
              ...
              children: [
                GestureDetector(
                  onTap: (){
                    setState(() {
                      crossClipState=(crossClipState==CrossClipState.showSecond)?CrossClipState.showFirst:CrossClipState.showFirst;
                      minuteColor=(minuteColor==beforeTap)?afterTap:minuteColor;
                      hourColor=(hourColor==afterTap)?beforeTap:hourColor;
                    });
                  },
                  child:Text('Minutes',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold,color: minuteColor),),
                ),
                GestureDetector(
                  onTap: (){
                    setState(() {
                      crossClipState=(crossClipState==CrossClipState.showFirst)?CrossClipState.showSecond:CrossClipState.showSecond;
                      minuteColor=(minuteColor==afterTap)?beforeTap:minuteColor;
                      hourColor=(hourColor==beforeTap)?afterTap:hourColor;
                    });
                  },
                  child:Text('Hours',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold,color: hourColor),),
                ),
              ],
            ),
          ),
          const Divider(thickness: 0.5,height:0,indent: 100,endIndent: 100,color: Colors.white,),
          ...
          AnimatedCrossClip(
            ...
            curve: Curves.linear,
            firstChild:const HorizontalTimeListView(itemExtent: 0.15, includeZero: true, lastNumber: 60),
            secondChild: const HorizontalTimeListView(itemExtent: 0.2, includeZero:true, lastNumber: 23),
          )
...
class HorizontalTimeListView extends StatelessWidget{
  ...
  const HorizontalTimeListView({super.key,required this.itemExtent,required this.includeZero,required this.lastNumber});
      ...
      child: SizedBox(
          ...
          child: ListWheelScrollViewX.useDelegate(
            onSelectedItemChanged: (int index){
              if(lastNumber==60){
                habitDuration=Duration(minutes:numberAtIndex(index),hours: 0); ///update minute
              }else{
                habitDuration=Duration(minutes:0,hours: numberAtIndex(index)); ///update hour
              }
            },
            ...
            controller: FixedExtentScrollController(initialItem: (lastNumber==60)?habitDuration.inMinutes:(habitDuration.inHours)),
...
class VerticalTimeListView extends StatelessWidget {
  ...
  const VerticalTimeListView({super.key,required this.screenWidthRatio,required this.includeZero,required this.lastNumber});
  ...
  Widget build(BuildContext context) {
    return  SizedBox(
      width:MediaQuery.of(context).size.width*screenWidthRatio,/// 1->0.5
      child: ListWheelScrollView.useDelegate(
        onSelectedItemChanged: (int index){
          if(lastNumber==60){
            habitStart= habitStart.replacing(minute: numberAtIndex(index)); ///update minute
          }else{
            habitStart=habitStart.replacing(hour: numberAtIndex(index)); ///update hour
          }
        },
        controller: FixedExtentScrollController(initialItem: (lastNumber==23)?habitStart.hour:(habitStart.minute)),
